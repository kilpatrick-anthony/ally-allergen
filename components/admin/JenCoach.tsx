        ...
        <div className="flex w-[350px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl" style={{ maxHeight: 'min(620px, calc(100vh - 160px))' }}>
          <div className="flex items-center gap-3 px-4 py-3 text-white" style={{ backgroundColor: character === 'ally' ? '#0e7066' : '#003842' }}>
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold leading-tight">Message {characterName}</p>
              <p className="text-xs leading-tight text-white/70">Contact the AllyJen team</p>
            </div>
            <button type="button" onClick={dismissCoach} className="rounded px-2 py-1 text-[11px] text-white/60 transition-colors hover:bg-white/10 hover:text-white" title="Hide until next login">Hide</button>
            <button type="button" onClick={() => setOpen(false)} className="rounded-full p-1 transition-colors hover:bg-white/10" aria-label={`Close ${characterName} message form`}><ChevronDown size={18} /></button>
          </div>
