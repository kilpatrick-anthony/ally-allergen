...
CREATE INDEX IF NOT EXISTS idx_kiosk_devices_business ON public.kiosk_devices(business_id);
CREATE INDEX IF NOT EXISTS idx_kiosk_devices_site ON public.kiosk_devices(site_id);
CREATE INDEX IF NOT EXISTS idx_kiosk_devices_online ON public.kiosk_devices(is_online, last_heartbeat);
CREATE INDEX IF NOT EXISTS idx_kiosk_devices_device_id ON public.kiosk_devices(device_id);

CREATE INDEX IF NOT EXISTS idx_offline_alerts_device ON public.device_offline_alerts(device_id);
CREATE INDEX IF NOT EXISTS idx_offline_alerts_unresolved ON public.device_offline_alerts(resolved) WHERE resolved = false;
CREATE INDEX IF NOT EXISTS idx_offline_alerts_business ON public.device_offline_alerts(business_id);

CREATE INDEX IF NOT EXISTS idx_heartbeats_device ON public.device_heartbeats(device_id, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_heartbeats_timestamp ON public.device_heartbeats(timestamp DESC);
...
BEGIN
  ...
  FOR device_record IN
    ...
    LEFT JOIN public.sites s ON d.site_id = s.id
  ...
  LOOP
    ...
    INSERT INTO public.device_offline_alerts (
      device_id,
      site_id,
      business_id,
      went_offline_at
...
CREATE OR REPLACE FUNCTION public.mark_device_online(p_device_id UUID)
RETURNS void AS $$
BEGIN
  UPDATE public.kiosk_devices
  SET is_online = true,
...
CREATE OR REPLACE VIEW public.device_status_summary
...
SELECT 
  ...
  (SELECT COUNT(*) FROM public.device_offline_alerts
...
REVOKE ALL ON TABLE public.kiosk_devices, public.device_offline_alerts, public.device_heartbeats
  FROM anon, authenticated;
GRANT ALL ON TABLE public.kiosk_devices, public.device_offline_alerts, public.device_heartbeats
  TO service_role;

REVOKE ALL ON FUNCTION public.check_offline_devices(), public.mark_device_online(uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.check_offline_devices(), public.mark_device_online(uuid)
  TO service_role;

REVOKE ALL ON TABLE public.device_status_summary FROM anon, authenticated;
GRANT SELECT ON TABLE public.device_status_summary TO service_role;
